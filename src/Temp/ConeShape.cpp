...
template<class T> std::unique_ptr<ConvexObject3D<T>> ConeShape<T>::toConvexObject(const nTransform<T>& transform) const {
	...
	return std::make_unique<nCone<T>>(radius * radiusScale, height * heightScale, coneOrientation, transform.getPosition(), transform.getOrientation());
}
