...
std::shared_ptr<const RayTestResult> PhysicsWorld::rayTest(const nRay<float>& ray) {
	std::scoped_lock<std::mutex> lock(mutex);
...
void PhysicsWorld::startPhysicsUpdate() {
	try {
		LogPrint("Physics thread started with time step of " + std::to_string(timeStep) + " sec");
		...
		LogError("Error cause physics thread crash: exception reported to main thread");
