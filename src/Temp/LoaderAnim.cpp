...
std::shared_ptr<ConstAnimation> LoaderUrchinAnim::loadFromFile(const std::string& filename, const std::map<std::string, std::string, std::less<>>&) {
	...
	FileReaderNextLine(file, buffer); //buffer = "hierarchy {"
	...
	FileReaderNextLine(file, buffer); //buffer = "bounds {"
	...
	FileReaderNextLine(file, buffer); //buffer = "baseframe {"
	...
	for (unsigned int frameIndex = 0; frameIndex < numFrames; ++frameIndex) {
		...
		FileReaderNextLine(file, buffer); // buffer = "frame ? {"
		...
		FileReaderNextLine(file, buffer); //buferf = "}"
	}
