#include "stdafx.h"
#include "ModelInstanceDisplayer.h"

ModelInstanceDisplayer::ModelInstanceDisplayer(const ModelSetDisplayer& modelSetDisplayer, DisplayMode displayMode, RenderTargetRef renderTarget, ShaderProgramRef shader) :
	isInitialized(false),
...
ModelInstanceDisplayer::~ModelInstanceDisplayer() {
	std::vector<nModel*> copiedInstanceModels = instanceModels;
	for (nModel* model : copiedInstanceModels) {
...
void ModelInstanceDisplayer::initialize() {
	...
	//	const nMesh& mesh = getReferenceModel().getMeshes()->getMesh(i);
...
void ModelInstanceDisplayer::updateMeshVertices(const nModel* model) const 
{
	assert(0);
	/*unsigned int meshIndex = 0;
	for (const auto& meshRenderer : meshRenderers) {
		if (model->isMeshUpdated(meshIndex)) {
			const nMesh& mesh = getReferenceModel().getMeshes()->getMesh(meshIndex);
...
void ModelInstanceDisplayer::updateMeshUv(const nModel* model) const 
{
	assert(0);
	//unsigned int meshIndex = 0;
	//for (const auto& meshRenderer : meshRenderers) {
...
void ModelInstanceDisplayer::updateMaterial(const nModel* model) const 
{
	assert(0);
	/*if (displayMode == DisplayMode::DEFAULT_MODE || displayMode == DisplayMode::DEFAULT_NO_INSTANCING_MODE) {
		unsigned int meshIndex = 0;
...
nModel& ModelInstanceDisplayer::getReferenceModel() const {
...
void ModelInstanceDisplayer::fillMaterialData(const nMesh& mesh) const 
{
	materialData.encodedEmissiveFactor = std::clamp(mesh.getMaterial().GetEmissiveFactor() / nMaterial::MAX_EMISSIVE_FACTOR, 0.0f, 1.0f);
...
std::span<nModel* const> ModelInstanceDisplayer::getInstanceModels() const {
	return instanceModels;
}

void ModelInstanceDisplayer::addInstanceModel(nModel& model) {
	if (instanceModels.empty()) {
...
void ModelInstanceDisplayer::removeInstanceModel(nModel& model) {
	model.detachModelInstanceDisplayer(*this);
	std::size_t erasedCount = std::erase_if(instanceModels, [&model](const nModel* m) {return m == &model; });
	if (erasedCount != 1) {
		LogError("Removing the instance model fail: " + model.getConstMeshes()->getId());
	}
}
...
void ModelInstanceDisplayer::registerRenderingModel(const nModel& model) {
#ifdef _DEBUG
