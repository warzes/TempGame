...
template<class T> nCone<T>::nCone() :
...
template<class T> nCone<T>::nCone(T radius, T height, typename ConeShape<T>::ConeOrientation coneOrientation, const Point3<T>& centerOfMass, const Quaternion<T>& orientation) :
	ConvexObject3D<T>(ConvexObjectType::CONE),
...
template<class T> T nCone<T>::getRadius() const {
	return coneShape.getRadius();
}

template<class T> T nCone<T>::getHeight() const {
	return coneShape.getHeight();
}

template<class T> typename ConeShape<T>::ConeOrientation nCone<T>::getConeOrientation() const {
	return coneShape.getConeOrientation();
}

template<class T> const Point3<T>& nCone<T>::getCenterOfMass() const {
	return centerOfMass;
}

template<class T> const Quaternion<T>& nCone<T>::getOrientation() const {
	return orientation;
}
...
*/
template<class T> const Vector3<T>& nCone<T>::getAxis(unsigned int index) const {
	return axis[index];
}

template<class T> Point3<T> nCone<T>::getCenter() const {
...
template<class T> Point3<T> nCone<T>::getSupportPoint(const Vector3<T>& direction) const {
...
template<class T> std::ostream& operator <<(std::ostream& stream, const nCone<T>& cone) {
...
//explicit template
template class nCone<float>;
template std::ostream& operator <<<float>(std::ostream&, const nCone<float>&);

template class nCone<double>;
template std::ostream& operator <<<double>(std::ostream&, const nCone<double>&);