...
template<class T> nPlane<T>::nPlane() :
...
*/
template<class T> nPlane<T>::nPlane(const Vector3<T>& normalizedNormal, T distanceToOrigin) :
...
template<class T> nPlane<T>::nPlane(const Point3<T>& p1, const Point3<T>& p2, const Point3<T>& p3) {
	buildFrom3Points(p1, p2, p3);
}

template<class T> nPlane<T>::nPlane(const Vector3<T>& normalizedNormal, const Point3<T>& point) {
	buildFromNormalAndPoint(normalizedNormal, point);
}

template<class T> void nPlane<T>::buildFrom3Points(const Point3<T>& p1, const Point3<T>& p2, const Point3<T>& p3) {
...
template<class T> void nPlane<T>::buildFromNormalAndPoint(const Vector3<T>& normalizedNormal, const Point3<T>& point) {
#ifdef _DEBUG
	T normalSquareLength = normal.squareLength();
	assert(normalSquareLength > (T)0.9996 || normalSquareLength < (T)1.0004);
#endif
...
template<class T> void nPlane<T>::reverse() {
	normal = -normal;
	d = -d;
}

template<class T> const Vector3<T>& nPlane<T>::getNormal() const {
	return normal;
}

template<class T> void nPlane<T>::setNormal(const Vector3<T>& normal) {
	this->normal = normal;
}

template<class T> T nPlane<T>::getDistanceToOrigin() const {
	return d;
}

template<class T> void nPlane<T>::setDistanceToOrigin(T d) {
	this->d = d;
}
...
*/
template<class T> T nPlane<T>::distance(const Point3<T>& p) const { //see http://fr.wikipedia.org/wiki/Distance_d%27un_point_%C3%A0_un_plan
...
template<class T> T nPlane<T>::distance(const Point4<T>& p) const {
...
*/
template<class T> T nPlane<T>::verticalDistance(const Point3<T>& p) const {
...
*/
template<class T> T nPlane<T>::horizontalDistance(const Point3<T>& p) const {
	Point3<T> planePoint = Point3<T>(normal * -d);
	return normal.dotProduct(p.vector(planePoint)) / normal.X;
}

template<class T> Point3<T> nPlane<T>::orthogonalProjection(const Point3<T>& p) const {
	Point3<T> planePoint = Point3<T>(normal * -d);
...
template<class T> Point3<T> nPlane<T>::intersectPoint(const Line3D<T>& line, bool& hasIntersection) const {
...
template<class T> std::ostream& operator <<(std::ostream& stream, const nPlane<T>& p) {
	return stream << "Plane normal: " << p.getNormal() << " | Distance to origin: " << p.getDistanceToOrigin();
}

//explicit template
template class nPlane<float>;
template std::ostream& operator <<<float>(std::ostream&, const nPlane<float>&);

template class nPlane<double>;
template std::ostream& operator <<<double>(std::ostream&, const nPlane<double>&);