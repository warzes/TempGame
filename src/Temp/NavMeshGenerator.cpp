...
NavMeshGenerator::NavMeshGenerator() :
	polygonMinDotProductThreshold(std::cos(AngleConverter<float>::toRadian(5.0/*ConfigService::instance().getFloatValue("navMesh.polygonRemoveAngleThresholdInDegree")*/))),
	polygonMergePointsDistanceThreshold(0.01/*ConfigService::instance().getFloatValue("navMesh.polygonMergePointsDistanceThreshold")*/),
	navMeshAgent(std::make_unique<NavMeshAgent>()),
	navMesh(std::make_shared<NavMesh>()),
	needFullRefresh(false),
	navigationObjects(AABBTree<std::shared_ptr<NavObject>>(0.2/*ConfigService::instance().getFloatValue("navMesh.polytopeAabbTreeFatMargin")*/)) {
...
void NavMeshGenerator::setNavMeshAgent(const NavMeshAgent& navMeshAgent) {
	...
	float navigationObjectsMargin = std::max(navigationObjectsJumpMargin, 0.2f/*ConfigService::instance().getFloatValue("navMesh.polytopeAabbTreeFatMargin")*/);
...
CSGPolygon<float> NavMeshGenerator::computePolytopeFootprint(const Polytope& polytopeObstacle, const PolytopeSurface& walkableSurface) const {
	...
	nPlane<float> walkablePlane = walkableSurface.getPlane(polytopeObstacle.getXZRectangle());
