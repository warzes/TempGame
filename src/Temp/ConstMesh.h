...
class ConstMesh {
public:
	ConstMesh(std::shared_ptr<nMaterial>, const std::vector<Vertex>&, std::vector<Point2<float>>,
		std::vector<unsigned int>, std::vector<Weight>, const std::vector<Bone>&);

	const std::shared_ptr<nMaterial>& getInitialMaterialPtr() const;
...
private:
	std::shared_ptr<nMaterial> initialMaterial;
