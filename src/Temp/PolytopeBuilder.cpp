...
PolytopeBuilder::PolytopeBuilder() :
	planeSurfaceSplitService(PlaneSurfaceSplitService(40.0/*ConfigService::instance().getFloatValue("navMesh.polytopeMaxSize")*/)),
	terrainSplitService(TerrainSplitService(40.0/*ConfigService::instance().getFloatValue("navMesh.polytopeMaxSize")*/))
{
}
...
std::vector<std::unique_ptr<Polytope>> PolytopeBuilder::buildExpandedPolytopes(const AIObject& aiObject, const NavMeshAgent& navMeshAgent) const {
	...
	for (auto& aiShape : aiObject.getShapes()) {
		...
		nTransform<float> shapeTransform = aiShape->hasLocalTransform() ? aiObject.getTransform() * aiShape->getLocalTransform() : aiObject.getTransform();
		...
		if (object->getObjectType() == ConvexObjectType::OBBOX) {
			...
			expandedPolytope = createExpandedPolytope(shapeName, static_cast<nCapsule<float>*>(object.get()), navMeshAgent);
		}
		else if (object->getObjectType() == ConvexObjectType::CONE) {
			expandedPolytope = createExpandedPolytope(shapeName, static_cast<nCone<float>*>(object.get()), navMeshAgent);
		}
			...
			expandedPolytope = createExpandedPolytope(shapeName, static_cast<nCylinder<float>*>(object.get()), navMeshAgent);
		}
		else if (object->getObjectType() == ConvexObjectType::SPHERE) {
			expandedPolytope = createExpandedPolytope(shapeName, static_cast<nSphere<float>*>(object.get()), navMeshAgent);
		}
		else {
...
std::vector<std::unique_ptr<Polytope>> PolytopeBuilder::buildExpandedPolytope(const AITerrain& aiTerrain, const NavMeshAgent& navMeshAgent) const {
	...
	auto terrainMaxWalkableSlope = AngleConverter<float>::toRadian(60.0/*ConfigService::instance().getFloatValue("navMesh.terrainMaxWalkableSlopeInDegree")*/);
...
std::unique_ptr<Polytope> PolytopeBuilder::createExpandedPolytope(std::string name, const nCapsule<float>* capsule, const NavMeshAgent& navMeshAgent) const {
...
std::unique_ptr<Polytope> PolytopeBuilder::createExpandedPolytope(std::string name, const nCone<float>* cone, const NavMeshAgent& navMeshAgent) const {
...
std::unique_ptr<Polytope> PolytopeBuilder::createExpandedPolytope(std::string name, const ConvexHull3D<float>* convexHull, const NavMeshAgent& navMeshAgent) const {
	std::map<std::size_t, nPlane<float>> expandedPlanes;
...
std::unique_ptr<Polytope> PolytopeBuilder::createExpandedPolytope(std::string name, const nCylinder<float>* cylinder, const NavMeshAgent& navMeshAgent) const {
...
std::unique_ptr<Polytope> PolytopeBuilder::createExpandedPolytope(std::string name, const nSphere<float>* sphere, const NavMeshAgent& navMeshAgent) const {
...
std::vector<Point3<float>> PolytopeBuilder::createExpandedPoints(const std::vector<Point3<float>>& sortedPoints, const NavMeshAgent& navMeshAgent) const {
	std::vector<nPlane<float>> sortedExpandedPlanes = createExpandedBoxPlanes(sortedPoints, navMeshAgent);
...
*/
std::vector<nPlane<float>> PolytopeBuilder::createExpandedBoxPlanes(const std::vector<Point3<float>>& sortedPoints, const NavMeshAgent& navMeshAgent) const {
	std::vector<nPlane<float>> expandedPlanes;
...
nPlane<float> PolytopeBuilder::createExpandedPlane(const Point3<float>& p1, const Point3<float>& p2, const Point3<float>& p3, const NavMeshAgent& navMeshAgent) const {
	nPlane plane(p1, p2, p3);
...
std::vector<Point3<float>> PolytopeBuilder::expandBoxPoints(const std::vector<nPlane<float>>& sortedExpandedPlanes) const {
	std::vector<Point3<float>> expandedPoints;
	expandedPoints.reserve(8);

	for (auto planeIndex : PLANE_INDEX_TO_POINTS) {
		const nPlane<float>& plane0 = sortedExpandedPlanes[planeIndex[0]];
		const nPlane<float>& plane1 = sortedExpandedPlanes[planeIndex[1]];
		const nPlane<float>& plane2 = sortedExpandedPlanes[planeIndex[2]];
