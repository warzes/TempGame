...
public:
	...
	void rayQuery(const nRay<float>&, std::vector<OBJ>&) const;
	void enlargedRayQuery(const nRay<float>&, float, const void*, std::vector<OBJ>&) const;
...
*/
template<class OBJ> void AABBTree<OBJ>::rayQuery(const nRay<float>& ray, std::vector<OBJ>& objectsAABBoxHitRay) const {
	browseNodes.clear();
	if (rootNode) [[likely]] {
		browseNodes.push_back(rootNode.get());
		}
...
*/
template<class OBJ> void AABBTree<OBJ>::enlargedRayQuery(const nRay<float>& ray, float enlargeNodeBoxHalfSize, const void* objectPtrToExclude,
