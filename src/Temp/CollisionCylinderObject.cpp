...
CollisionCylinderObject::CollisionCylinderObject(float outerMargin, float radius, float height, typename CylinderShape<float>::CylinderOrientation cylinderOrientation,
	...
	cylinderObject(nCylinder<float>(radius, height, cylinderOrientation, centerOfMass, orientation)) {
...
nCylinder<float> CollisionCylinderObject::retrieveCylinder() const {
	return nCylinder<float>(getRadius(), getHeight(), getCylinderOrientation(), getCenterOfMass(), getOrientation());
}
