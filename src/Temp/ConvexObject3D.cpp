...
template<class T> bool ConvexObject3D<T>::collideWithSphere(const nSphere<T>&) const {
	throw std::runtime_error("Collide with sphere: unsupported");
}

template<class T>  bool ConvexObject3D<T>::collideWithRay(const nRay<T>&) const {
