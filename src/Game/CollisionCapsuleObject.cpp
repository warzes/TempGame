...
CollisionCapsuleObject::CollisionCapsuleObject(float outerMargin, float radius, float cylinderHeight, typename CapsuleShape<float>::CapsuleOrientation capsuleOrientation,
	...
	capsuleObject(nCapsule<float>(radius, cylinderHeight, capsuleOrientation, centerOfMass, orientation)) {
...
nCapsule<float> CollisionCapsuleObject::retrieveCapsule() const {
	return nCapsule<float>(getRadius(), getCylinderHeight(), getCapsuleOrientation(), getCenterOfMass(), getOrientation());
}
