...
template<class T> std::unique_ptr<ConvexObject3D<T>> CylinderShape<T>::toConvexObject(const nTransform<T>& transform) const {
	...
	return std::make_unique<nCylinder<T>>(radius * radiusScale, height * heightScale, cylinderOrientation, transform.getPosition(), transform.getOrientation());
}
