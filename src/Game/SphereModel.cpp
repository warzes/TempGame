...
SphereModel::SphereModel(nSphere<float> sphere, unsigned int slices) :
	slices(slices) {
	spheres.push_back(std::move(sphere));
}

SphereModel::SphereModel(std::vector<nSphere<float>> spheres, unsigned int slices) :
...
std::vector<Point3<float>> SphereModel::retrieveVertexArray(std::vector<uint32_t>&) const {
	...
	for (const nSphere<float>& sphere : spheres) {
...
PrimitiveTopology SphereModel::getShapeType() const {
