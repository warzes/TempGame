...
BodyAABBTree::BodyAABBTree() :
	AABBTree<std::shared_ptr<AbstractBody>>(0.2/*ConfigService::instance().getFloatValue("broadPhase.aabbTreeFatMargin")*/),
...
void BodyAABBTree::controlBoundaries(const AABBNode<std::shared_ptr<AbstractBody>>& leafNode) const {
	...
	if (bodyAABBox.getMax().Y < minYBoundary) {
		...
		if (!body->isStatic()) {
			LogWarning("Body " + body->getId() + " is below the limit of " + std::to_string(minYBoundary) + ": " + StringUtil::toString(body->getTransform().getPosition()));
