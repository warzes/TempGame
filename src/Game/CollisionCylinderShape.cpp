#include "stdafx.h"
#include "CollisionCylinderShape.h"
...
void CollisionCylinderShape::computeSafeMargin() {
	...
	float maximumMarginPercentage = 0.3;// ConfigService::instance().getFloatValue("collisionShape.maximumMarginPercentage");
...
std::unique_ptr<CollisionShape3D> CollisionCylinderShape::scale(const Vector3<float>& scale) const {
	...
	if (!MathFunction::isEqual(scale[(heightAxis + 1) % 3], scale[(heightAxis + 2) % 3], 0.01f)) {
		LogWarning("Cylinder cannot be correctly scaled with " + StringUtil::toString(scale) + ". Consider to use another shape.");
	}
