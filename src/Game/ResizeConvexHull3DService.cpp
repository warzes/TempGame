...
template<class T> std::unique_ptr<ConvexHullShape3D<T>> ResizeConvexHull3DService<T>::resizeConvexHullShape(const ConvexHullShape3D<T>& originalConvexHullShape, T distance) {
	std::map<std::size_t, nPlane<T>> planes = buildPlanesFromConvexHullShape(originalConvexHullShape);
...
template<class T> std::unique_ptr<ConvexHullShape3D<T>> ResizeConvexHull3DService<T>::resizeConvexHullShape(const ConvexHullShape3D<T>& originalConvexHullShape, const std::map<std::size_t, nPlane<T>>& expandedPlanes) {
	...
	for (const auto& itPoint : originalConvexHullShape.getConvexHullPoints()) {
		std::vector<nPlane<T>> threePlanes = findThreeNonParallelPlanes(itPoint.second.triangleIndices, expandedPlanes);
...
template<class T> std::unique_ptr<ConvexHull3D<T>> ResizeConvexHull3DService<T>::resizeConvexHull(const ConvexHull3D<T>& originalConvexHull, const std::map<std::size_t, nPlane<T>>& expandedPlanes) {
...
template<class T> std::map<std::size_t, nPlane<T>> ResizeConvexHull3DService<T>::buildPlanesFromConvexHullShape(const ConvexHullShape3D<T>& convexHull) {
	std::map<std::size_t, nPlane<T>> planes;
	...
	for (const auto& itTriangles : convexHull.getIndexedTriangles()) {
		...
		planes.emplace(itTriangles.first, nPlane<T>(point1, point2, point3)); //plane is built with normal outside convex hull
	}

	return planes;
}

template<class T> void ResizeConvexHull3DService<T>::shiftPlanes(std::map<std::size_t, nPlane<T>>& planes, T distance) {
	for (auto& itPlanes : planes) {
		itPlanes.second.setDistanceToOrigin(itPlanes.second.getDistanceToOrigin() - distance);
	}
}

template<class T> std::vector<nPlane<T>> ResizeConvexHull3DService<T>::findThreeNonParallelPlanes(const std::vector<std::size_t>& planeIndices, const std::map<std::size_t, nPlane<T>>& allPlanes) {
	constexpr float PARALLEL_COMPARISON_TOLERANCE = 0.01f;

	std::vector<nPlane<T>> nonParallelPlanes;
	nonParallelPlanes.reserve(3);

	const nPlane<T>& plane1 = allPlanes.at(planeIndices[0]);
	for (std::size_t i = 1; i < planeIndices.size(); ++i) {
		const nPlane<T>& plane2 = allPlanes.at(planeIndices[i]);
		...
		for (std::size_t j = i + 1; j < planeIndices.size(); ++j) {
			const nPlane<T>& plane3 = allPlanes.at(planeIndices[j]);
