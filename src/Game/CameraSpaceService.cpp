...
CameraSpaceService::CameraSpaceService(const nCamera& camera) :
	camera(camera) {
...
*/
nRay<float> CameraSpaceService::screenPointToRay(const Point2<float>& screenPoint, float rayLength) const {
	...
	return nRay<float>(rayStart, rayEnd);
}
