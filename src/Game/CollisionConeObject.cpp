...
CollisionConeObject::CollisionConeObject(float outerMargin, float radius, float height, typename ConeShape<float>::ConeOrientation coneOrientation,
	...
	coneObject(nCone<float>(radius, height, coneOrientation, centerOfMass, orientation)) {
...
nCone<float> CollisionConeObject::retrieveCone() const {
	return nCone<float>(getRadius(), getHeight(), getConeOrientation(), getCenterOfMass(), getOrientation());
}
